...
export default function AdminWithdrawals({history}) {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        const response = await serverRequest(token).get('/admin/withdrawals');
      ...
      <Paper className={classes.paper}>
        ...
        <Table>
          ...
          <TableBody>
            ...
            {withdrawals.map(row => (
              <TableRow key={row.id} hover onClick={() => history.push(`/admin/withdrawals/${row.id}`)} className={classes.tablerow}>
